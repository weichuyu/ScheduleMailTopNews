...
import com.yu.tools.repo.BatchGoogleNews_USRepo;
 ...
 */
@Service
public class FetchGoogleNewUSServiceImpl implements IFetchGoogleNewUSService {
    private static Logger logger = Logger.getLogger(FetchGoogleNewUSServiceImpl.class);
    @Resource
    private MyHttpClientUtil myHttpClientUtil;

    @Resource
    private BatchGoogleNews_USRepo batchGoogleNews_USRepo;
    ...
    public String getNewsHtml(Boolean writeDataBase, String batchId) {
        String html = myHttpClientUtil.getDataFromURL_GET("https://news.google.com/topics/CAAqJggKIiBDQkFTRWdvSUwyMHZNRFZxYUdjU0FtVnVHZ0pWVXlnQVAB?hl=en-US&gl=US&ceid=US%3Aen", "GBK",false);
        ...
        if (writeDataBase && result != null && result.data!=null) {
            for (FetchGoogleNewUSServiceImpl.ModelWrapper ob : result.data) {
                BatchGoogleNews_US s = new BatchGoogleNews_US();
    ...
    public Map<String,Object> getCurrentCompareMailHtml() {
        ...
        query.setBatchtype("GoogleNews_US");
        ...
        String uuid1 = null;
        String uuid2 = null;
        List<BatchGoogleNews_US> list1 = null;
        List<BatchGoogleNews_US> list2 = null;
        ...
        for (int i = 0; i < 2; i++) {
            ...
            List<BatchGoogleNews_US> temp = batchGoogleNews_USRepo.findByBatchid(result.get(i).getBatchId());
            if (i == 0) {
                list1 = temp;
                nowDateStr = str;
        ...
        for(BatchGoogleNews_US ob : list1){
            map1.put(ob.getKeyword(),true);
        }
        for(BatchGoogleNews_US ob : list2){
